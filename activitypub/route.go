...
func Route(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("GET /.activitypub/actor/", RouteActor)
...
func RouteActor(w http.ResponseWriter, r *http.Request) {
	...
	if len(sep) < 4 {
		slog.Warn("activitypub.RouteActor", "error", "actor format error")
		w.WriteHeader(http.StatusNotFound)
		json.NewEncoder(w).Encode(map[string]string{"error": "actor format error"})
		return
	}
	username := sep[3]
